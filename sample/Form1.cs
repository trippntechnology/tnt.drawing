...
public partial class Form1 : Form
{
    ...
    public Form1()
    {
        ...
        _canvas = new Canvas(toolStripContainer1.ContentPanel)
        {
            ...
            {
                TNTLogger.Info($"OnFeedbackChanged: {feedback.Hint}");
        ...
        var toolStrips = new[] { fileToolStrip, toolStrip2, toolStrip1 };
        int x = 0;
    ...
    {
        var modeGroupManager = new ToolStripItemRadioGroupManager()
        {
            OnClick = item =>
            {
        ...
        modeGroupManager.Create<Select>(new ToolStripItem[] { toolStripButton1, toolStripMenuItem1 }, Resources.Images.SelectorTool).Also(group => { group.Tag = _selectMode; });
        modeGroupManager.Create<Line>(new ToolStripItem[] { toolStripButton2, toolStripMenuItem2 }, Resources.Images.PolylineTool).Also(group => { group.Tag = _lineMode; });
        modeGroupManager.Create<Group.Rectangle>(new ToolStripItem[] { toolStripButton3, toolStripMenuItem3 }, Resources.Images.RectangleTool).Also(group => { group.Tag = _rectangularMode; });

        var menuGroupManager = new ToolStripItemGroupManager()
        {
            OnClick = item =>
            {
                switch (item)
        ...
        menuGroupManager.Create<Delete>(new ToolStripItem[] { deleteToolStripMenuItem, deleteToolStripButton }).Also(group =>
          {
              var selected = _canvas.Layers.OfType<ObjectLayer>()?.First()?.Let(objectLayer => objectLayer.GetSelected());
              group.Enabled = selected?.Count() > 0;
          }
        );
    }
